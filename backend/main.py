...
VALIDATION TASKS:
1. IDENTICAL INDICATORS: Find comparison blocks where TWO indicators of the SAME TYPE have IDENTICAL parameters.
   - This is an ERROR for crossover strategies (e.g., SMA(14) > SMA(14) makes no sense)
   - FIX: Make one Fast (shorter period) and one Slow (longer period)
   - Standard: SMA Fast=10/Slow=20, EMA Fast=12/Slow=26, RSI Fast=7/Slow=14
...
2. COMPARISON LOGIC: Ensure crossover comparisons are logical:
   ...
   - If indicators are in wrong order, swap the comparison operator

3. POLISH: Ensure proper NAME fields ("Fast SMA", "Slow SMA"), proper timeframes, valid mutation attributes.
