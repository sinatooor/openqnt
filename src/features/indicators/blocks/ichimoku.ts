...
Blockly.Blocks['ta_ichimoku'] = {
    init: function () {
        const config = getIndicatorConfig('ichimoku');
        this.appendDummyInput()
            .appendField("Ichimoku")
            .appendField(new Blockly.FieldTextInput("Ichimoku"), "NAME")
            .appendField(new Blockly.FieldDropdown(
                config?.components?.map(c => [c.label, c.value]) || []
            ), "COMPONENT")
            .appendField("TF:").appendField(new Blockly.FieldDropdown(TIMEFRAME_OPTIONS), "PERIOD").appendField(createGearSettingsButton('ichimoku'));
        this.setOutput(true, "TAValue");
        this.setStyle('ta_blocks');
        this.setTooltip("Ichimoku Kinko Hyo");
        this.indicatorName = 'ichimoku';
        this.indicatorParams = getDefaultParams('ichimoku');
