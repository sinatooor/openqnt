...
Blockly.Blocks['osma'] = {
    init: function () {
        const config = getIndicatorConfig('osma');
        this.appendDummyInput()
            .appendField("OsMA")
            .appendField(new Blockly.FieldTextInput("OsMA"), "NAME")
            .appendField(new Blockly.FieldDropdown(
                config?.components?.map(c => [c.label, c.value]) || []
            ), "COMPONENT")
            .appendField("TF:").appendField(new Blockly.FieldDropdown(TIMEFRAME_OPTIONS), "PERIOD").appendField(createGearSettingsButton('osma'));
        this.setOutput(true, "TAValue");
        this.setStyle('ta_blocks');
        this.setTooltip("Moving Average of Oscillator");
        this.indicatorName = 'osma';
        this.indicatorParams = getDefaultParams('osma');
