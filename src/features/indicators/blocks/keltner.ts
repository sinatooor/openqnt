...
Blockly.Blocks['ta_keltner'] = {
    init: function () {
        const config = getIndicatorConfig('keltner');
        this.appendDummyInput()
            .appendField("Keltner Channels")
            .appendField(new Blockly.FieldTextInput("Keltner"), "NAME")
            .appendField(new Blockly.FieldDropdown(
                config?.components?.map(c => [c.label, c.value]) || []
            ), "COMPONENT")
            .appendField("TF:").appendField(new Blockly.FieldDropdown(TIMEFRAME_OPTIONS), "PERIOD").appendField(createGearSettingsButton('keltner'));
        this.setOutput(true, "TAValue");
        this.setStyle('ta_blocks');
        this.setTooltip("Keltner Channels");
        this.indicatorName = 'keltner';
        this.indicatorParams = getDefaultParams('keltner');
