...
Blockly.Blocks['volumes'] = {
    init: function () {
        const config = getIndicatorConfig('volumes');
        this.appendDummyInput()
            .appendField("Volumes")
            .appendField(new Blockly.FieldTextInput("Volumes"), "NAME")
            .appendField(new Blockly.FieldDropdown(
                config?.components?.map(c => [c.label, c.value]) || []
            ), "COMPONENT")
            .appendField("TF:").appendField(new Blockly.FieldDropdown(TIMEFRAME_OPTIONS), "PERIOD").appendField(createGearSettingsButton('volumes'));
        this.setOutput(true, "TAValue");
        this.setStyle('ta_blocks');
        this.setTooltip("Volumes");
        this.indicatorName = 'volumes';
        this.indicatorParams = getDefaultParams('volumes');
