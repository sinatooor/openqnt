    ...
    <div className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-1.5">
          {isLive ? (
            <Button
              size="sm"
              variant="outline"
              className="h-7 px-2 text-[11px] gap-1 border-red-500/30 text-red-500 hover:text-red-500"
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto">
        {!run ? (
          ...
          <div className="p-3 space-y-2">
            ...
            {isLive && (
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground pl-1 pt-1">
      ...
      {run && run.status !== 'running' && run.conclusion && (
        <div className="border-t border-border/60 px-3 py-2 bg-muted/30">
          ...
          <p className="text-[12px] text-foreground/80 line-clamp-3">
...
const RunStatusPill = ({ status }: { status: 'running' | 'success' | 'error' | 'cancelled' }) => {
  if (status === 'running')
    return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider bg-amber-500/15 text-amber-500">
        <Loader2 className="w-2.5 h-2.5 animate-spin" /> Live
      </span>
    );
  if (status === 'success')
    return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider bg-emerald-500/15 text-emerald-500">
        <CheckCircle2 className="w-2.5 h-2.5" /> Done
      </span>
    );
  if (status === 'cancelled')
    return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider bg-muted/60 text-foreground/70">
        <Ban className="w-2.5 h-2.5" /> Cancelled
      </span>
    );
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider bg-red-500/15 text-red-500">
...
const EmptyState = ({ agentLabel }: { agentLabel: string }) => (
  <div className="h-full flex items-center justify-center p-8 text-center">
    <div>
      <p className="text-[13px] text-foreground/80 mb-1">{agentLabel} is idle</p>
