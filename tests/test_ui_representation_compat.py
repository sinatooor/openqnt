...
def test_json_structure_compliance():
    ...
    # Load reference
    # Use path relative to root where this test now lives
    ref_path = os.path.join(os.path.dirname(__file__), "..", "backend", "backtest_result.json")
    if not os.path.exists(ref_path):
        # Fallback if running from backend folder or different structure
        ref_path = os.path.join("backend", "backtest_result.json")
