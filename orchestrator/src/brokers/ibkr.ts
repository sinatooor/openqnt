...
export class IbkrClient implements BrokerClient {
    ...
    async connect(_apiKey: string, _apiSecret?: string): Promise<void> {
        // TODO: Implement Interactive Brokers Client Portal API connect
        this.connected = true;
        logger.info({ broker: this.name }, 'IBKR connected (Stub)');
    }
    ...
    async submitOrder(order: BrokerOrder): Promise<BrokerOrderResult> {
        logger.info({ order, broker: this.name }, 'Submitting order to IBKR (Stub)');
        return {
    ...
    async cancelOrder(orderId: string): Promise<void> {
        logger.info({ orderId, broker: this.name }, 'Cancelling order in IBKR (Stub)');
    }
    ...
    async closePosition(symbol: string): Promise<BrokerOrderResult> {
        logger.info({ symbol, broker: this.name }, 'Closing position in IBKR (Stub)');
        return {
            orderId: `ibkr_close_${Date.now()}`,
            status: 'accepted',
            filledQuantity: 0,
            filledPrice: 0,
        };
